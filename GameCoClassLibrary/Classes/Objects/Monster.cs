...
namespace GameCoClassLibrary.Classes
{
  ...
  internal class Monster
  {
    ...
    {
      get { return new Point(_arrayPos.X, _arrayPos.Y); }
    }
    ...
    {
      get { return new PointF(_canvaPos.X, _canvaPos.Y); }
    }
    ...
    {
      get { return !_currentBaseParams.Invisible; }
    }
    ...
    /// The half sizes.
    /// </value>
    internal static int[] HalfSizes { private get; set; }
      ...
      if(flag)
      {
        switch(GetDirection)
        {
          ...
          case MonsterDirection.Up:
            ...
            if((_wayPos == _way.Count - 1)
               || (_canvaPos.Y <= ((_way[_wayPos + 1].Y * Settings.ElemSize + Settings.ElemSize / 2))))
            {
              return true;
            }
    ...
    private void EffectsImpact()
    {
      ...
      {
        effect.DoEffect(ref _currentBaseParams.CanvasSpeed, ref _currentBaseParams.HealthPoints,
                        ref _currentBaseParams.Armor);
      }
      _effects.RemoveAll(x => x.DestroyMe);
      if(_currentBaseParams.HealthPoints > 0)
      {
        return;
      }
    ...
    internal void ShowMonster(IGraphic canva, Point visibleStart, Point visibleFinish)
    {
      ...
      canva.DrawImage(tmp, (int)(realX - (tmp.Width / 2.0) * Scaling), (int)(realY - (tmp.Height / 2.0) * Scaling),
                      (int)(tmp.Width * Scaling), (int)(tmp.Height * Scaling));
      ...
      if(r != 0 || g != 0 || b != 0)
      {
        canva.FillEllipse(new SolidBrush(Color.FromArgb((byte)r, (byte)g, (byte)b)), realX - 5 * Scaling,
                          realY - 5 * Scaling, 10 * Scaling, 10 * Scaling);
      }
      ...
      int currentHPLineLength =
        (int)(Math.Round(_currentBaseParams.HealthPoints * 100.0 / _params.Base.HealthPoints) / HPBarLen);
      ...
      switch(GetDirection)
      {
        case MonsterDirection.Left:
        case MonsterDirection.Right:
          canva.DrawLine(Helpers.BlackPen, Convert.ToInt32(realX - (HPBarLen / 2) * Scaling), realY,
                         Convert.ToInt32(realX + (HPBarLen / 2) * Scaling), realY);
          if(currentHPLineLength == 0)
          {
            break;
          }
          canva.DrawLine(Helpers.GreenPen, Convert.ToInt32(realX - (HPBarLen / 2) * Scaling), realY,
                         Convert.ToInt32(realX + (currentHPLineLength / 2.0) * Scaling), realY);
          break;
        case MonsterDirection.Up:
        case MonsterDirection.Down:
          canva.DrawLine(Helpers.BlackPen, realX, Convert.ToInt32(realY + (HPBarLen / 2) * Scaling), realX,
                         Convert.ToInt32(realY - (HPBarLen / 2) * Scaling));
          if(currentHPLineLength == 0)
          {
            break;
          }
          canva.DrawLine(Helpers.GreenPen, realX, Convert.ToInt32(realY + (HPBarLen / 2) * Scaling), realX,
                         Convert.ToInt32(realY - (currentHPLineLength / 2.0) * Scaling));
          break;
      }
    }
    ...
    internal void GetDamadge(int damadge, eModificatorName modificator = eModificatorName.NoEffect,
                             bool reduceable = true /*may be reduced by armor*/)
    {
    ...
    internal void Load(BinaryReader loadStream)
    {
      ...
      for(int i = 0; i < n; i++)
      {
        _effects.Add(AttackModificators.CreateEffectByID((eModificatorName)loadStream.ReadInt32(),
                                                         loadStream.ReadInt32()));
      }
    }
  }
}