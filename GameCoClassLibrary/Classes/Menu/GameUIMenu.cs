        ...
        delegate(out Point location, ref Size size)
          {
            switch(buttonType)
            {
              case Button.StartLevelEnabled:
                location = new Point(
                  Convert.ToInt32((Settings.BreakupLineXPosition - Settings.DeltaX
                                   - Res.Buttons[Button.StartLevelDisabled].Width) * Scaling),
                  Convert.ToInt32((Settings.DeltaY * 2 + Settings.MapAreaSize + Res.Buttons[Button.SmallScale].Height)
                                  * Scaling));
                break;
              ...
              case Button.SmallScale:
                location = new Point(
                  Convert.ToInt32((Settings.DeltaX + Res.Buttons[Button.BigScale].Width
