...
namespace GameCoClassLibrary.Classes
{
  ...
  internal sealed class GraphicEngine
  {
    ...
    /// Background color
    /// </summary>
    private readonly Color _backgroundColor = Color.Silver;
    ...
    {
      _graphObject.RemoveImageFromCache(_constantMapImage);
    ...
    internal void Show(Game gameObj)
    {
...

      {
        ...
        _graphObject.DrawLine(new Pen(new SolidBrush(Color.White), Settings.PenWidth * gameObj.Scaling),
                              ...
                              new Point(Convert.ToInt32(Settings.BreakupLineXPosition * gameObj.Scaling),
                                        Convert.ToInt32(Settings.WindowHeight * gameObj.Scaling)));
      ...
      if(gameObj.Paused)
      {
        _graphObject.MakeGray(0, 0, Convert.ToInt32(Settings.WindowWidth * gameObj.Scaling),
                              Convert.ToInt32(Settings.WindowHeight * gameObj.Scaling));
      }
    }
    ...
    private void MapAreaShowing(Game gameObj)
    {
      ...
      _graphObject.Clip =
        new Rectangle(Convert.ToInt32(Settings.DeltaX * gameObj.Scaling),
                      ...
                      Convert.ToInt32((gameObj.Map.VisibleXFinish - gameObj.Map.VisibleXStart) * Settings.ElemSize
                                      * gameObj.Scaling),
      ...
      else if(gameObj.TowerMapSelectedID != -1)
      {
        ShowSquareAndCircleAtTower(gameObj,
                                   new Point(
                                     gameObj.Towers[gameObj.TowerMapSelectedID].ArrayPos.X - gameObj.Map.VisibleXStart,
                                     gameObj.Towers[gameObj.TowerMapSelectedID].ArrayPos.Y - gameObj.Map.VisibleYStart),
                                   gameObj.Towers[gameObj.TowerMapSelectedID].CurrentTowerParams.AttackRadius,
                                   Color.White);
      }
      ...
      _graphObject.Clip = new Rectangle(0, 0, Convert.ToInt32(Settings.WindowWidth * gameObj.Scaling),
                                        Convert.ToInt32(Settings.WindowHeight * gameObj.Scaling));
    }
    ...
    private void ShowTowerParams(Game gameObj)
    {
      ...
      Size textSize = TextRenderer.MeasureText(strToShow,
    ...
    //This region have so small methods, that's for future development
    ...
    private void ShowMoney(Game gameObj)
    {
      ...
      _graphObject.DrawString(gameObj.Gold.ToString(CultureInfo.InvariantCulture),
                              ...
                              Helpers.BlackBrush,
                              new Point(
                                Convert.ToInt32((Settings.BreakupLineXPosition + Res.MoneyPict.Width + Settings.DeltaX)
                                                * gameObj.Scaling),
                                Convert.ToInt32(Settings.DeltaY * gameObj.Scaling)));
    }
